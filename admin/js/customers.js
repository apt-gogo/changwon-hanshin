...
(() => {
  ...
  function setViewStatus(nextStatus) {
    ...
    el.completedButton.textContent = isConfirmed ? '미확인 고객' : '확인완료 고객';
  ...
  function renderCustomers(customers) {
    ...
    el.rows.innerHTML = customers.map(item => {
      const action = canConfirm
        ? `<button class="confirm-button" type="button" data-confirm-id="${item.id}" data-confirm-name="${escapeHtml(item.customer_name)}">확인완료</button>`
    ...
    el.mobileCards.innerHTML = customers.map(item => {
      ...
      const action = canConfirm
        ? `<button class="confirm-button mobile-confirm-button" type="button" data-confirm-id="${item.id}" data-confirm-name="${escapeHtml(item.customer_name)}">확인완료</button>`
  ...
  async function handleConfirmClick(event) {
    ...
    try {
      ...
      console.error(error);
      setStatus('확인완료 처리 중 오류가 발생했습니다.', 'error');
